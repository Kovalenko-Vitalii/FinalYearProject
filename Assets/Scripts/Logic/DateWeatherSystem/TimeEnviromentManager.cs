...
public class TimeEnviromentManager : MonoBehaviour, IPlayerTick, ISaveable
{
    ...
    [SerializeField] private string id = "TimeEnviromentManager";
    ...
    private void NormalizeTimeAndDay()
    {
        while (currentMinutes >= MinutesPerDay)
        {
            currentMinutes -= MinutesPerDay;
            currentDay++;
        }

        while (currentMinutes < 0f)
        {
            currentMinutes += MinutesPerDay;
    ...
    {
        currentMinutes += hours * MinutesPerHour + minutes;
        NormalizeTimeAndDay();
        ApplySun();
    }
