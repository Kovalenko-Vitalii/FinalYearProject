    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 ProcessClouds(float3 dir, fixed3 skyCol)
            {
                ...
                float2 windDir = _CloudWindDirection.xy;
                float windLen = length(windDir);
                windDir = windLen > 1e-4 ? (windDir / windLen) : float2(1.0, 0.0);

                float2 windRot = float2(windDir.x * c - windDir.y * s, windDir.x * s + windDir.y * c);
